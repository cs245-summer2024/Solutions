...
public class BST<T extends Comparable> {
    ...
    public List<T> rangeQuery(T lowerBound, T upperBound){
        ...
        rangeHelper(root, lowerBound, upperBound, values);
        return values;
    }

    public void rangeHelper(BSTNode node, T lowerBound, T upperBound, List<T> values){
        if(node == null){
            return;
        } else if(node.item.compareTo(lowerBound) < 0) {
            rangeHelper(node.right, lowerBound, upperBound, values);
        } else if(node.item.compareTo(upperBound) > 0){
            rangeHelper(node.left, lowerBound, upperBound, values);
        } else {
            values.add((T)node.item);
            rangeHelper(node.right, lowerBound, upperBound, values);
            rangeHelper(node.left, lowerBound, upperBound, values);
        }
    }
